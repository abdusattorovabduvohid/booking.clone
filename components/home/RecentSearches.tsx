...
import { fetchRecentSearches } from "@/lib/backend-data";
...
export async function RecentSearches() {
  ...
  return (
    <section className="mb-10">
      <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white">{t("recent_searches_title")}</h2>
      <div className="flex gap-4 overflow-x-auto snap-x no-scrollbar pb-4">
        {searches.map((item: any) => (
          <div key={item.id} className="snap-start shrink-0 w-64 md:w-72 bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.1)] dark:shadow-none dark:border dark:border-gray-700 relative">
            ...
            <div className="p-4">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 truncate">{item.city}, {item.country}</p>
